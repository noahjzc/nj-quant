...
from backtesting.risk.stop_loss_strategies import StopLossStrategies
...
class DailyRotationEngine:
    ...
    def __init__(self, config: RotationConfig, start_date: str, end_date: str,
                 data_provider=None, ranker=None):
        ...
        Args:
            ...
            ranker: 排名器实例。None 则用默认 SignalRanker；
                    传入 MLRanker 使用 ML 模型排名。
        """
        self.config = config
        ...
        self.ranker = ranker or SignalRanker(config.rank_factor_weights, config.rank_factor_directions)
    ...
    def _check_and_sell(
        ...
        for stock_code, position in list(self.positions.items()):
            ...
            # T+1 约束检查
            stock_row = self._today_df[self._today_df['stock_code'] == stock_code]
            pct_chg = stock_row['pct_chg'].values[0] if not stock_row.empty and 'pct_chg' in stock_row.columns else 0.0
            ok, reason = self.market_constraints.can_sell(position, pd.Timestamp(date_str), current_prices.get(stock_code, 0.0), pct_chg)
            if not ok:
                if reason == 'T+1约束':
                    continue
                else:
    ...
    def _execute_buy(
        ...
        # 特殊因子需要计算，其余直接从缓存批量选取
        computed = {}
        for factor in self.ranker.required_features:
            if factor == 'RET_20':
                computed[factor] = cdf['ret_20'].fillna(0.0).astype(float)
                ...
                computed[factor] = pd.Series(np.where(mv > 0, np.log(mv), np.nan), index=cdf.index)
            elif factor == 'WR_10':
                computed[factor] = cdf['wr_10'].fillna(0.0).astype(float)
            elif factor == 'WR_14':
                computed[factor] = cdf['wr_14'].fillna(0.0).astype(float)

        cached = [f for f in self.ranker.required_features if f not in computed and f in cdf.columns]
        factor_df = cdf[cached].copy() if cached else pd.DataFrame(index=cdf.index)
        for f, s in computed.items():
            factor_df[f] = s
        for f in self.ranker.required_features:
            if f not in computed and f not in cdf.columns:
                factor_df[f] = 0.0
        ...
        for stock_code in ranked:
            ...
            # 计算成本
            cost = self.cost_model.buy_cost(price, shares).total

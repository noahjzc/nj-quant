...
**Architecture:** 扫描数据源目录，解析每个CSV文件，使用`session.merge()`批量写入数据库。
...
Run: `python -m py_compile back_testing/data/sync/import_index_from_csv.py`
Expected: 无输出（语法正确）

- [ ] **Step 3: 提交**

```bash
git add back_testing/data/sync/import_index_from_csv.py
git commit -m "feat(data): add index CSV import script
...
1. **Spec覆盖**: 只有1个Task，覆盖了"扫描目录→解析CSV→批量写入index_daily"的完整流程。
2. **占位符扫描**: 无TBD/TODO，代码完整可运行。
3. **类型一致性**: `IndexDaily`字段名与CSV列映射正确（money→turnover）。

...
def compute_sharpe(equity: List[float], periods_per_year: int = 252) -> float:
    ...
    if len(returns) < 2 or returns.std(ddof=1) < 1e-10:
        return 0.0
...
def run_walk_forward(
    ...
    for i, (train_s, train_e, test_s, test_e) in enumerate(windows):
        ...
        test_sharpe = _evaluate_on_test(best_config, test_start_str, test_end_str, data_provider, ranker)
...
def _evaluate_on_test(config: RotationConfig, start: str, end: str,
                      data_provider=None, ranker=None) -> float:
...
def _select_by_robustness(study: optuna.Study, base_config: RotationConfig,
                          start_date: str, end_date: str,
                          output_dir: str = None,
                          data_provider=None,
                          ranker=None):
    """从 Top 5 参数中按综合评分（Sharpe × 0.6 + 稳定性 × 0.4）选出最稳健参数。

    在全部 Trial 完成后执行，不修改 Optuna 优化流程本身。
    对每组候选参数跑完整的 SensitivityAnalyzer（每个参数 ±20%），
    从中选出 Sharpe 高且对参数扰动不敏感的解。

    Args:
        study: 已完成的 Optuna Study
        base_config: 基础配置
    ...
    # 按 Trial value (Sharpe) 降序取 Top 5
...
示例:
  ...
  python optimization/optuna/run_daily_rotation_optimization.py \\
    ...
    if args.verbose:
        ...
        logging.getLogger('strategy.rotation').setLevel(logging.DEBUG)
